  ...
  methods: {
    ...
    schedulingToggleClick(date, data) {
        if (!this.schedulingDateIsDisabled) {
          // this.$message({message: '请先点击编辑按钮'});
          return;
        }
        // 有值的时候，点击的时候，清空
        if (this.schedulingObj[data.day] || (this.schedulingObj[data.day] && this.schedulingObj[data.day] !== '')) {
          this.schedulingObj[data.day] = '';
        }
        else {
          let week = this.$moment(data.day).format('d');
          if (week === '0' || week === '6') {
              this.schedulingObj[data.day] = '工';
          }
          else {
              this.schedulingObj[data.day] = '非';
          }
        }

...
export default {
    ...
    methods: {
        initChart() {
            ...
            let min = this.getMinutes(this.min);
            let max = this.getMinutes(this.max);
            var option;
            option = {
                tooltip: {
                    trigger: 'item',
                    formatter: params => {
                        if (!params.length) {
                            ...
                            console.log(val, 'val');
                            return `背景周期：${val ? this.getTimeByMinutes(
                                val[0]
                            ) : '-'}<br/>优化周期：${val ? this.getTimeByMinutes(val[2]) : '-'}
                            <br/>优化幅度：${val ? this.getTimeByMinutes(val[2]) : '-'}`
                            ;
                        }
                    },

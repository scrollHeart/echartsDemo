  ...
  methods: {
    ...
    getCellClass({ text, type}) {
      ...
      if (type === 'current') {
        ...
        let mm = date.slice(date.slice(date.indexOf('-') + 1, date.lastIndexOf('-')));
        ...
        else if (date.slice(0, date.lastIndexOf('-')) === this.$moment().format('YYYY-MM') && Number(date.split('-')[2]) < Number(this.$moment().format('DD'))) {
          classes.push('is-past');
        }
        else if (date.split('-')[0] === this.$moment().format('YYYY') && Number(mm) < Number(this.$moment().format('MM'))) {
          classes.push('is-past');
        }
        console.log(date.split('-')[0] === this.$moment().format('YYYY'))
        console.log(Number(mm) < Number(this.$moment().format('MM')))
      }
      else {

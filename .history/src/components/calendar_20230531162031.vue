...
export default {
  ...
  methods: {
    pickDay(day, isCurMonth) {
      // 点击编辑按钮后，可操作日历配置, 未切换到其他月份
        if (!this.configBtnObj.editClicked && isCurMonth == undefined) {
          this.$message({message: '请先点击编辑按钮！', duration: 0, customClass: 'default-custom-msg'});
          return;
        }
    ...
    focusYear() {
      ...
      if (this.configBtnObj.editClicked) {
        ...
        this.$message({message: '请先保存当前配置内容！', duration: 0, customClass: 'default-custom-msg'});
        return;
      }
    },
  ...
  computed: {
    ...
    isAbledClick() {
      console.log(this.valueYear === `${(new Date()).getFullYear()}` && Number(this.$moment().format('MM')) > Number(this.curMonth))

    ...
    methods: {
        ...
        updateChart() {
            ...
            series = this.parsedChartData.map((item, index) => {
                ...
                if (setNameArr.indexOf(optionName) < 0) {
                    setNameArr.push(optionNameArr[index]);
                    posiYObj[index] = - height / 2;
                }
                else {
                    console.log('重复了', index)
            ...
            this.chartRef.on('legendselectchanged', (e) => {
                ...
                // selected为true表示选中，false表示未选中
                // 关闭或开启某个策略，和其有同步时段的策略图形位置要发生变化
                console.log(series, 'series--changed');
                this.series[3].renderItem = (params, api) => {
                        let categoryIndex = api.value(1);
                        let start = api.coord([api.value(0), categoryIndex]);
                        let end = api.coord([api.value(2), categoryIndex]);
                                ...
                                children: [
                                    {
                                        ...
                                        style: {
                                            fill: 'white',
                                            stroke: this.chartOptions[3] && this.chartOptions[3].borderColor || '',
                                            lineWidth: 1,
                                            opacity: 0.3,
                                        },
                                    },
                                    {
                                        ...
                                        shape: rectShape,
                                        style: {
                                            fill: this.chartOptions[3] && this.chartOptions[3].color || '',
                                            opacity: 0.5,
                                        },
                                        emphasis: {
                                            style: {
                ...
                this.series[0].data = [];
                this.series[2].data = [];
                console.log(this.series, 'this.series--after');
                this.chartOptions[0].color = 'transparent';
                this.chartOptions[2].color = 'transparent';
                console.log(this.chartOptions, 'chartOptions');
                // option.legend.selected[e.name] = e.selected;

    ...
    methods: {
        ...
        parseChartData() {
            ...
            for (let j = 0; j < chartData.length; j++) {
                ...
                chartData[j].forEach((s, i) => {
                    // labels.push(this.parseLabel(s.name));
                    let lastTo = -1;
                    (s.values || []).forEach(d => {
                        const {key, value} = d;
            ...
            //     });
            // });
            // // console.log('解析：', this.chartData, labels);
            // this.labels = labels;
            // this.parsedChartData = data;
        },
        // 获取时间的数字表示（0-1440）
        getMinutes(time) {
            if (time.indexOf(':') > 0) {
        ...
        updateChart() {
            ...
            series = this.parsedChartData.map((item, index) => {
                return {
                    name: this.chartOptions[index].name || '',
                    type: 'custom',
                    // selectedMode: 'single',
                                ...
                                children: [
                                    {
                                        ...
                                        style: {
                                            fill: 'white',
                                            stroke: this.chartOptions[index].borderColor || '',
                                            lineWidth: 1,
                                            opacity: 0.3,
                                        },
                                    },
                                    {
                                        ...
                                        shape: rectShape,
                                        style: {
                                            fill: this.chartOptions[index].color || '',
                                            opacity: 0.5,
                                        },
                                        emphasis: {
                                            style: {
